...
DoC   Pdbmat:
DoC
DoC   Computes the mass-weighted second derivatives energy matrix,
DoC   using Tirion's model, that is, an elastic network model (ENM).
DoC   In such models, close particles (atoms) are linked by springs.
DoC
DoC   In Tirions's model, springs are set between atoms less than 
DoC   CUToff Angstroms away from each others.
DoC
DoC   In Hinsen's version, springs are weighted by exp(-dij/rkh)**2.
DoC   Herein, both kind of typical distances (cutoff & rkh) can be mixed.
DoC
DoC   Goal: Computing the low-frequency (collective) normal modes 
DoC   of vibration of the system.
DoC
DoC   To do so, the matrix produced by pdbmat has to be diagonalized
DoC   with a program like DIAGSTD or, for large systems, DIAGRTB,
DoC   BLZPACK, etc.
DoC
DoC=====================================================================
DoC
DoC   INPUT: 
DoC   ******
DoC  -A parameter file named pdbmat.dat.  
DoC
DoC   Note that each run of pdbmat produces a pdbmat.dat_run file,
DoC   where parameter values are shown (and shortly commented).
DoC   pdbmat.dat_run can be modified and used as a pdbmat.dat file, 
DoC   for further runs. 
DoC   So, the simplest is to compile and run pdbmat...
DoC   and have a look at the pdbmat.dat_run file so produced.
DoC   If you want to see there the syntax of other available commands,
DoC   just raise the PRINting level.
DoC
DoC  -Among the parameters: 
DoC
DoC  *The name of a file with the coordinates of the system, 
DoC   in FREE or PDB (protein data bank) format.
DoC
DoC   Free format: x, y, z, mass.
DoC   PDB  format: Only lignes with the ATOM keyword are considered.
DoC        Masses can be given in the Bfactors column.
DoC   Note that masses can be read, but they are not required.
DoC   In the later case, they are all set to 1.0 (as often done).
DoC
DoC  *A way to identify pairs of neighbors:
DoC   Either a CUTOFF value (standard Tirion's model)
DoC   Or a file with a list of pairs of neighbors.
DoC   Format of each ligne of this later file: atom-number atom-number
DoC
DoC   Alternatively, Hinsen's version can be used.
DoC
DoC   For one-atom-per-residue protein models, typical values are:
DoC   Tirion's distance cutoff = 10-12 Angstroms.
DoC   Hinsen's typical range   = 3 Angstroms.
DoC
DoC   OUTPUT:
DoC   *******
DoC
DoC  -Default output matrix filename:
DoC   Formatted file: pdbmat.sdijf 
DoC   Binary    file: pdbmat.sdijb 
DoC
DoC   This is a matrix in format: i, j, non-zero-i-j-matrix-element
DoC
DoC  -Output coordinate filename (in free format):
DoC   pdbmat.xyzm
DoC
DoC   This is a coordinate file with, for each atom: 
DoC   x, y, z, mass, block-number
DoC   For a pdb file, the block-number is the amino-acid residue number
DoC  (it is of use only in other programs like DIAGRTB).
DoC
DoC   More specialized ones:
DoC   ----------------------
DoC
DoC  -VMD-command filename: 
DoC   pdbmat.vmd
DoC
DoC   This is a command file for vizualising the elastic network
DoC   with VMD, the Visual Molecular Dynamics program (v1.8):
DoC   vmd -e pdbmat.vmd 
DoC   But if you just want to vizualise a standard ENM (defined with
DoC   a cutoff distance), there are more efficient (faster) ways.
DoC
DoC  -Molscript-command-file:
DoC   pdbmat.molscript
DoC
DoC   This is a command file for vizualising the elastic network
DoC   with Molscript (v2.1).
DoC   molscript < pdbmat.molscript > pdbmat.ps
DoC   But if you just want to vizualise a standard ENM (defined with
DoC   a cutoff distance) with Molscript, there are much faster ways.
DoC
DoC.....................................................................
      program pdbmat
      implicit none
...
DoC   This is a fortran 77 program, so it has predefined:
DoC
DoC   **************
DoC   MEMORY LIMITS:
DoC   **************
DoC
DoC   NATMAX  :  Maximum number of atoms (particles).
DoC   NRESMX  :  Maximum number of residues (groups of particles).
DoC   NVOISMX :  Maximum number of pairs of neighbors.  
   
      parameter( NATMAX=250000 )
      parameter( NRESMX=250000 )
      parameter( NVOISMX=1000000 )
    
DoC   Increase them if needed. To (re)compile pdbmat, type:
DoC   make pdbmat
DoC   or:
DoC   g77 -o pdbmat pdbmat.f
DoC   or use your favorite fortran compiler (instead of g77).
DoC.....................................................................
DoC
DoC   ABOUT Tirion's model:
DoC   *********************
DoC
DoC   Principe du modele (Tirion, 1996): 
DoC   
DoC   Tous les atomes a moins de "cutoff" les uns des autres 
DoC   sont supposes lies par des ressorts, qui ont tous
DoC   la meme raideur.
DoC   Simplification supplementaire par rapport au modele initial: 
DoC   les atomes sont supposes avoir tous la meme taille
DoC  (le cutoff est le meme pour toutes les paires d'atomes).
DoC   On peut de plus poser qu'ils ont tous la meme masse.
DoC   Sinon, celles-ci sont lues dans la colonne des 
DoC   facteurs B du fichier pdb.
DoC
DoC   Principaux resultats:
DoC
DoC   Les modes de vibration de basse frequence obtenus
DoC   a partir d'un tel modele sont tres voisins de ceux
DoC   obtenus avec un modele beaucoup plus detaille, tels
DoC   ceux utilises lors des etudes de Dynamique Moleculaire.
DoC
DoC   Dans le cas ou le mouvement fonctionnel d'une proteine est un 
DoC   mouvement d'ensemble (collectif), on constate qu'il peut tres 
DoC   souvent etre decrit comme une combinaison lineaire de quelques
DoC   uns de ces modes (de un a trois).
DoC
DoC   Principaux avantages:
DoC
DoC   Pas besoin de prendre en compte tous les atomes.
DoC   Pas besoin de minimisation d'energie prealablement
DoC   au calcul des modes de vibration (E=0 par construction).
DoC
DoC.....................................................................
DoC
DoC   MAIN REFERENCES:
DoC   ****************
DoC
DoC   1) M.M. Tirion (1996):
DoC  "Large amplitude elastic motions in proteins from
DoC   a single-parameter, atomic analysis",
DoC   Phys. Rev. letters vol.77(9), p1905-1908.
DoC
DoC   2) K. Hinsen (1998):
DoC  "Analysis of domain motions by approximate normal mode calculations"
DoC   Proteins vol.33, p417-429.
DoC
DoC   3) F. Tama, Y.H. Sanejouand (2001):
DoC  "Conformational change of proteins arising 
DoC   from normal modes calculations"
DoC   Protein Engineering vol.14, p1-6.
DoC
DoC   4) A.R. Atilgan, S.R. Durell, R.L. Jernigan, M.C. Demirel,
DoC   O. Keskin, I. Bahar (2001):
DoC  "Anisotropy of Fluctuation Dynamics of Proteins with an Elastic 
DoC   Network Model"
DoC   Biophys Journal vol.80, p.505-515.
DoC
DoC   5) S. Nicolay, Y.H. Sanejouand (2006):
DoC  "Functional modes of proteins are among the most robust"
DoC   Phys. Rev. letters vol.96, p078104.
DoC
DoC   In case of problem, feel free to contact: 
DoC   Yves-Henri.Sanejouand@ens-lyon.fr
DoC  (bug reports may help you, but also others)
DoC
DoC.....................................................................
...
c
DoC   New features since the previously released version (v3.50):
DoC  -The list of pairs of neighbors can be read in a file.
DoC  -The elastic network can be visualized, using VMD or Molscript.
DoC  -The output format has been extended.
DoC  -Hinsen's weight added.
DoC  -Bond-dihedral energy term added (1-4 bond, dihedral-like). 
DoC  -DoCumentation included in the code.
DoC
DoC.....................................................................
...
DoC   Covalent bond detection: through a distance-cutoff criterium.
...
DoC      Even when the CONsecutif keyword is used.
...
DoC   Bond-ANGLes are allowed only if there are BONDs.
...
DoC   Bond-DIHEdrals are allowed only if there are BONDs and ANGLes.
...
DoC         Hinsen's version: force constant * exp(-(dist/rkh)**2.d0)
DoC        (except for topological bond force constants, e.g. ANGLes).
